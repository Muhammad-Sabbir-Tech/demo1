import React from 'react';
import AOS from 'aos';

function ServiceComponent(props) {
    AOS.init()
    return (
        <>
            <section className="ftco-section bg-light">
                <div className="container">
                    <div className="row justify-content-center" data-aos="fade-up" data-aos-offset="200" data-aos-easing="ease" data-aos-duration="600">
                        <div className="col-lg-5 heading-section text-center mb-5" >
                            <span className="subheading">Services</span>
                            <h2 className="mb-4">Our Exclusive Services We Offer For You</h2>
                        </div>
                    </div>
                    <div data-aos="fade-up" data-aos-offset="200" data-aos-easing="ease" data-aos-duration="600" className="row justify-content-center g-2">
                        <div className="col-md-6 col-lg-3 text-center d-flex align-items-stretch" >
                            <div className="flow-wrap">
                                <div className="icon">
                                    <span className="flaticon-creative-design" />
                                </div>
                                <div className="text">
                                    <h2>Creative Design</h2>
                                    <p className="mb-4">Far far away, behind the word mountains, far from the countries Vokalia.</p>
                                </div>
                            </div>
                        </div>
                        <div className="col-md-6 col-lg-3 text-center d-flex align-items-stretch" >
                            <div className="flow-wrap">
                                <div className="icon">
                                    <span className="flaticon-bullhorn" />
                                </div>
                                <div className="text">
                                    <h2>Digital Marketing</h2>
                                    <p className="mb-4">Far far away, behind the word mountains, far from the countries Vokalia.</p>
                                </div>
                            </div>
                        </div>
                        <div className="col-md-6 col-lg-3 text-center d-flex align-items-stretch" >
                            <div className="flow-wrap">
                                <div className="icon">
                                    <span className="flaticon-design" />
                                </div>
                                <div className="text">
                                    <h2>Application Design</h2>
                                    <p className="mb-4">Far far away, behind the word mountains, far from the countries Vokalia.</p>
                                </div>
                            </div>
                        </div>
                        <div className="col-md-6 col-lg-3 text-center d-flex align-items-stretch"  >
                            <div className="flow-wrap">
                                <div className="icon">
                                    <span className="flaticon-web-programming" />
                                </div>
                                <div className="text">
                                    <h2>Web Development</h2>
                                    <p className="mb-4">Far far away, behind the word mountains, far from the countries Vokalia.</p>
                                </div>
                            </div>
                        </div>
                        <div className="col-md-6 col-lg-3 text-center d-flex align-items-stretch" >
                            <div className="flow-wrap">
                                <div className="icon">
                                    <span className="flaticon-branding" />
                                </div>
                                <div className="text">
                                    <h2>Branding</h2>
                                    <p className="mb-4">Far far away, behind the word mountains, far from the countries Vokalia.</p>
                                </div>
                            </div>
                        </div>
                        <div className="col-md-6 col-lg-3 text-center d-flex align-items-stretch" >
                            <div className="flow-wrap">
                                <div className="icon">
                                    <span className="flaticon-vector" />
                                </div>
                                <div className="text">
                                    <h2>Graphic Design</h2>
                                    <p className="mb-4">Far far away, behind the word mountains, far from the countries Vokalia.</p>
                                </div>
                            </div>
                        </div>
                        <div className="col-md-6 col-lg-3 text-center d-flex align-items-stretch" >
                            <div className="flow-wrap">
                                <div className="icon">
                                    <span className="flaticon-email-marketing" />
                                </div>
                                <div className="text">
                                    <h2>Email Marketing</h2>
                                    <p className="mb-4">Far far away, behind the word mountains, far from the countries Vokalia.</p>
                                </div>
                            </div>
                        </div>
                        <div className="col-md-6 col-lg-3 text-center d-flex align-items-stretch" >
                            <div className="flow-wrap">
                                <div className="icon">
                                    <span className="flaticon-3d-design" />
                                </div>
                                <div className="text">
                                    <h2>Web Design</h2>
                                    <p className="mb-4">Far far away, behind the word mountains, far from the countries Vokalia.</p>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </section>
        </>
    );
}

export default ServiceComponent;